...
def generate_similar_questions(question: str) -> list:
    ...
    model = genai.GenerativeModel(MODEL_NAME)
    res = model.generate_content(prompt)
    items = [line.strip() for line in res.text.strip().splitlines() if line.strip()]
    return items[:3]

def check_answer(question: str, student_answer: str, last_context: str = "") -> dict:
    """
    Ask Gemini to judge whether the student's answer is correct.
...
"""
    ...
    # Extract a short feedback (everything after first newline or after the RESULT line)
    parts = text.split("\n", 1)
    feedback = parts[1].strip() if len(parts) > 1 else ""
    correct_answer = None
    if not correct:
        # try to extract answer by simple regex of numbers in feedback
